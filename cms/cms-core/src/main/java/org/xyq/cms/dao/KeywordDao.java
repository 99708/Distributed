...
public class KeywordDao extends BaseDao<Keyword> implements IKeywordDao {
	...
	public void clearNoUseKeyword() {
		...
		Set<String> ks = getExistKeyword2Map().keySet();
	...
	public List<Keyword> listKeywordByCon(String con) {
		String hql = "from Keyword " +
				"where name like '%"+con+"%' or nameFullPy like '%"+con+"%' " +
	...
	public void addOrUpdate(String name) {
		Keyword keyword = (Keyword) this.queryObject("from Keyword where name=?", name);
		if(keyword == null){
			keyword = new Keyword();
	...
	public List<String> listStringByCon(String con) {
		String hql = "select name from Keyword " +
				"where name like '%"+con+"%' or nameFullPy like '%"+con+"%' " +
				"or nameShortPy like '%"+con+"%'";
